...
namespace ZBDevice
{
	...
	int openSerial()
	{
		...
		struct termios opt;
		tcflush(fd, TCIOFLUSH);    //��մ��ڽ��ջ�����	
		tcgetattr(fd, &opt);       //��ȡ���ڲ���opt	
		cfsetospeed(&opt, B115200);//���ô������������
		cfsetispeed(&opt, B115200);//���ô������벨����
		//��������λ��
		opt.c_cflag &= ~CSIZE;
		opt.c_cflag |= CS8;
		//У��λ
		opt.c_cflag &= ~PARENB;
		opt.c_iflag &= ~INPCK;
		opt.c_cflag &= ~CSTOPB;      //����ֹͣλ
        opt.c_lflag &= ~(ICANON | ECHO | ECHOE); //�������ã��������ֲ���ǿ�Ƽӻ��з�
        //���0x03 0x13����ʾ����
    ...
    SYSTEMTIME ZhFlightControl::systemTime()
    {
        ...
        //systime.wMinute = 8;
        //systime.wSecond = 28;
        //systime.wMilliseconds = 690;
        //m_flightCtrlInfo.systime = systime;
        //
     ...
     * @brief �������
     * @param result ������� ��������
     * @param fAzi_origin �豸�ϱ��Ƕ�
     * @param fEle_origin �豸�ϱ��ĸ����ǣ�Ĭ��0
     * @param fRoll �������̺���Ƕ�
     * @param fPitch �������̸�����
     * @param fYaw �������̷����+��ƫ��
     */
